...
* __Extreme cases__: We are looking for cases that are breaking away from the rest of the cloud - a rule of thumb would be coordinates above 2, but it it depends more on the amount of space between the extreme points and the rest of the cloud. Often the problem is in two categories that necessarily overlap or missing categories. The solution can be to set some categories as passive.  
...
map.ca.base() + add.category.relations(r, color = "red") + add.categories(r)
...
add.pem.triads <- function(triads, triad.edges, mapping = aes(shape = category, color = triad), mapping_triads = aes(color = triad, alpha = pem),  
                           point.size = 3, pem.cut = 0.5, 
                           ...){
  
  tec    <- triad.edges %>% group_by(year, triad) %>% summarise(edges = sum(pem > 0), .groups = "drop_last") %>% ungroup()
  triads <- left_join(triads, tec, by = c("year", "triad"))
  ...
  mapping_triads <- add_modify_aes(mapping_triads, aes(x = X, xend = Xend, y = Y, yend = Yend))
  
  o <- list()
  
  o$segment       <- geom_segment(data = triad.edges, mapping = mapping_triads, size = 0.3, ...)
  
  o$background <- geom_point(data = triads, mapping = mapping, size = point.size + 2, shape = 21, alpha = 0.8, fill = "white", color = "white")
  o$ring       <- geom_point(data = triads %>% filter(edges >= 3), mapping = mapping, size = point.size + 2, shape = 21, alpha = 0.8, fill = "white")
  o$points     <- geom_point(data = triads, mapping = mapping, size = point.size, ...)
